...
def vendor_pypi_package(spec, dest):
    # ═══ mp_Net-Core 本地修補 ═══
    # 目標套件已存在且未設 VIPER_FORCE_PIP 時直接跳過 pip：
    # 1) 本專案環境 pip 暫存目錄不可寫 (OSError Errno 13)，套件由 vendor 腳本預先放置；
    # 2) 之後重建不需網路/pip。想強制更新請設環境變數 VIPER_FORCE_PIP=1。
    pkg_dir = path.join(dest, spec.split("==", 1)[0].lower().replace("-", "_"))
    if not os.environ.get("VIPER_FORCE_PIP") and path.isdir(pkg_dir):
        print(f"ViperIDE: skip pip vendor for {spec} (already present)")
        return
    # ═══ 以上為本地修補，以下為 upstream 原邏輯 ═══
