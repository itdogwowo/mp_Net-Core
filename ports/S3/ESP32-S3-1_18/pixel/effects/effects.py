...
# ── ports/S3 bench 專用：測試行程效果（motor_home / motor_test_cycle）───
# 段型：{type:keep|sine|stop, side:A|B, speed:1-100, frames}；波形一样用
# 框架整數正弦（_sine_bell_raws）__init__ 預計成表，frame 只查表。

class _uart_motor_timeline:
    """分段行程共用實作（不登記）：預計 raw表。"""
    ...
    def __init__(self, name, params=None):
        ...
        segs = []
        acc = 0
        for seg in params.get('program') or []:
            frames = max(0, int(seg.get('frames', 0) or 0))
            if frames <= 0:
                continue
            kind = str(seg.get('type', 'stop'))
            side = str(seg.get('side', 'A')).upper()
            spd = max(0, min(int(seg.get('speed', 64) or 0), 100))
            segs.append((acc, acc + frames, kind, side, spd))
            acc += frames
        self._total = acc
        if not segs:
            raise ValueError('{}: program 沒有有效段'.format(name))
        table = [128 << 4] * self._total
        for start, end, kind, side, speed in segs:
            L = end - start
            if kind == 'keep':
                raw = _motor_value(side, speed)
                for p in range(start, end):
                    table[p] = raw << 4
            elif kind == 'sine':
                prof = _sine_bell_raws(L, speed)
                for p in range(L):
                    raw = _motor_value(side, prof[p])
                    table[start + p] = raw << 4
            else:
                for p in range(start, end):
                    table[p] = _STOP_RAW << 4
    ...
    def frame(self, t):
        t = int(t)
        v12 = self._table[t] if t < self._total else (_STOP_RAW << 4)
        buf = self._buf
        for i in range(self.pixel_n):
            buf[i] = v12
        return buf
...
class motor_home(_uart_motor_timeline):
    """歸位：A側(收)全速 2s(±100幀) → 停 0.5s。開機 auto 一次。"""


class motor_test_cycle(_uart_motor_timeline):
    """測試行程：B側正弦伸 10s(500f) → A側全速收 12s(600f) → 停。"""


for _cls in (motor_home, motor_test_cycle):
    register(_cls)
