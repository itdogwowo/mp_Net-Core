...
class ScheduleTask(Task):
    ...
    #        `it.get("bus", "vBus")` 永遠回預設值。
    #        → 所以這次移除是純清理，沒有行為變更。
    #   ⚠️ 本 port 是 slave 的 fork，沒有 lib/sys/signal_router.py ——
    #      這裡的 Router 說明是同步 slave 的設計；本 port 的 Router 區塊
    #      目前仍是惰性的（見 todo/03_signal_router.md）。
