# ports/P4/ESP32-P4-ETH_mp3 — ESP32-P4 乙太網 + 音效播放板

## 用途
用 I2S + PCM5102 播放音效：PC 端先把 MP3 預轉 WAV，slave 端經 dj_task 串流播放。
播放模式一律放 `pixel/modes/`（本專案所有效果/音效模式都屬 pixel 系列）。
...
| `pixel/modes/sfx_play.json` | mode id **3**：播 `E_44100_16_2.wav`（map 留空＝無燈效、只出聲） |
...
| id | mode | payload |
|---|---|---|
| 3 | sfx_play（音效） | `00 03 00 00 FF` |

- 延遲：schedule 的 `ms` 欄位控制「開機後第幾 ms 觸發」；音效本身延遲改
  `pixel/modes/sfx_play.json` 的 `delay_ms`。
...
python -m mpremote connect COM fs cp ports/P4/ESP32-P4-ETH_mp3/pixel/modes/sfx_play.json :/pixel/modes/sfx_play.json
python -m mpremote connect COM fs cp ports/P4/ESP32-P4-ETH_mp3/schedule.json :/schedule.json
# 3) WAV 音源放 SD 卡 /sd/audio/E_44100_16_2.wav
# 4) RESET
```
